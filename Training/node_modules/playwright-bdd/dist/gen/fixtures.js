...
function extractFixtureNames(fn) {
    return (0, fixtureParameterNames_1.fixtureParameterNames)(fn).filter((name) => !(0, autoInject_1.isBddAutoInjectFixture)(name));
}
...
function extractFixtureNamesFromFnBodyMemo(fn) {
    ...
    if (!fnWithFixtures[bodyFixturesSymbol]) {
        fnWithFixtures[bodyFixturesSymbol] = extractFixtureNamesFromFnBody(fn).filter((name) => !(0, autoInject_1.isBddAutoInjectFixture)(name));
    }
